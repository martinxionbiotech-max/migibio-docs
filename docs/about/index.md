---
title: "About Migibio: Company, Credentials & Quality System"
date: 2026-08-13
description: "Company profile of Guangzhou Magic Biotech Co., Ltd. (Migibio) — founding, certifications (ISO9001, GMP), R&D capabilities, manufacturing quality system, patents, and export footprint."
---

# About Migibio: Company, Credentials & Quality System

This page documents who Migibio is and — more importantly — the credentials and quality infrastructure behind the technical claims made throughout this knowledge base.

## Company Profile

| Field | Detail |
|-------|--------|
| Legal name | Guangzhou Magic Biotech Co., Ltd. |
| Brand | Migibio |
| Founded | November 2017 |
| Headquarters | Guangzhou Science City, Huangpu District, Guangzhou, Guangdong, China |
| Core business | POCT veterinary diagnostic analyzers + fluorescence immunochromatographic (FICT) test reagents |
| Export footprint | 30+ countries |
| Intellectual property | 30 core patents |
| Contact | Martin.Wong — martinwang2024@gmail.com · +86 13323237275 (WhatsApp) |

## Certifications & Compliance

| Certification | Scope | Relevance |
|---------------|-------|-----------|
| **ISO 9001** | Quality management system | Entire organization — design, production, service |
| **GMP** | Good Manufacturing Practice | Reagent manufacturing environment and process control |

These certifications are the operational backbone of the QC workflows documented in [Domain 07 · Quality Control & Compliance](/domain-07-quality-control-compliance/).

## R&D & Manufacturing Capability

- **Core technology platform**: Fluorescence Immunochromatographic Technology (FICT) — quantitative, instrument-read immunoassays with pg/ml-scale sensitivity.
- **Instrument line**: FIA680 (single-channel) and FIA880 (six-channel) fluorescence immunoassay analyzers.
- **Reagent line**: Quantitative rapid tests for renal (SDMA), cardiac (NT-proBNP), inflammatory (CRP/SAA), endocrine (T4/TSH, cortisol, progesterone, relaxin), and infectious-disease (CPV/CDV/FPV/FeLV/FIV) markers.
- **In-house validation**: Each assay undergoes analytical validation (LOD, LOQ, precision, linearity, accuracy, interference) per the methodology documented in our [Research Center](https://research.migibio.net/analytical-performance/).

## Product & Quality Documentation

Every product ships with:

- **IFU** (Instructions for Use) — the legally controlling document for each assay.
- **COA** (Certificate of Analysis) — lot-specific performance data.

This knowledge base complements — never replaces — the IFU. Where they differ, the IFU governs.

## The Three-Site Ecosystem

| Site | Purpose |
|------|---------|
| [migibio.net](https://migibio.net) | Main website — products, marketing, company blog |
| [docs.migibio.net](https://docs.migibio.net) | This knowledge base — *how to use* the technology correctly |
| [research.migibio.net](https://research.migibio.net) | Research center — *why the technology is trustworthy* (methodology, validation, comparative studies) |
| [data.migibio.net](https://data.migibio.net) | Data hub — *the numbers* (reference ranges, assay performance, benchmarks) |

## Editorial Standards

All content on this site is produced and reviewed under a documented process. See the [Editorial & Review Policy](/editorial-policy/).

---

*For procurement, OEM/ODM, and distributor inquiries, see [Domain 10 · B2B Resources](/domain-10-b2b-resources/).*
